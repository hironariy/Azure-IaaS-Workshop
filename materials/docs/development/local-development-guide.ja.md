...
> **位置づけ:** このページは、アプリケーションをローカルで変更・検証したい開発者向けの任意資料です。通常のワークショップ受講者が Azure へデプロイする本線では、ローカル Node.js、Docker、Azure CLI、OpenSSL の準備は必須ではありません。受講者の標準手順は [受講者クイックスタート](../learner/learner-quickstart.ja.md)、[Day 1: Azure リソースデプロイ](../learner/day-1-deployment-checklist.ja.md)、[Day 1: アプリデプロイ](../learner/day-1-app-deployment.ja.md) を参照してください。
